...
$(function() {
  $('#conn-refresh').click( function(){
    com.list(function (err, ports) {
      if (err){ T.msg = err; $( "#dialog" ).click(); console.log('Error: ' + T.msg); }
      else{
  ...
  $('#conn-connect').click(function(){
    ...
    port.open(function () {
      T.msg = 'Connected'; $( "#dialog" ).click(); console.log('Error: ' + T.msg);

      port.on('error', function(err) { T.msg = err; $( "#dialog" ).click(); console.log('Error: ' + err); });
      port.on('data', function(data) { console.log('data: ' + data) });

      port.write("ID?\r\n", function(err, results) {
        if(err){ T.msg = err; $( "#dialog" ).click(); console.log('Error: ' + err); }
